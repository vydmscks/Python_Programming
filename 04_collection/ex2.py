...
a = ["짝" if x % 2 == 0 else "홀" for x in range(1, 11) ]
print(a)
...
# 길이가 5 이상인 이름만 뽑기
overFive = [name for name in names if len(name) >= 5]
print(overFive)
...
result = [s for s in scores if s >= 60]
...
result = [word for word in words if word.find("n") >= 0]  
result = [word for word in words if word.count() >= 0]  
