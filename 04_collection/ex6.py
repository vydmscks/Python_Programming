...
print(dict(Counter(words)))
                                            # :white_check_mark: {'apple': 3, 'banana': 2, 'cherry': 1}


# :two: 60점 이상인 경우 합격 설정하기
scores = {"국어": 85, "영어": 50, "수학": 95, "과학": 40, "사회": 72}
result = {k: "합격" for k, v in scores.items() if v >= 60}
print(result)
                                            # :white_check_mark: {'국어': '합격', '수학': '합격', '사회': '합격'}


# :three: 과목 리스트와 점수 리스트로 딕셔너리 만들기
subjects = ["국어", "영어", "수학"]
grades = [90, 80, 100]
result = dict(zip(subjects, grades))
print(result)
                                            # :white_check_mark: {'국어': 90, '영어': 80, '수학': 100}


# :four: 기존 재고에 입고 내역을 합치기 (이미 있는 상품은 합산, 새 상품은 추가)
...
print(stock)
                                            # :white_check_mark: {'연필': 10, '지우개': 9, '노트': 10, '볼펜': 12}```